fetch("http://localhost:3000/api/investment/Gold")
  .then(res => res.json())
  .then(data => {
    if (!Array.isArray(data) || data.length === 0) return;
    const labels = data.map(x => x.sub_category);
    const values = data.map(x => parseFloat(x.total));
    window.makeDoughnut("goldDoughnut", labels, values, "Gold Type");
  });

fetch("http://localhost:3000/api/buy-sell/Gold")
  .then(res => res.json())
  .then(data => {
    if (!Array.isArray(data) || data.length === 0) return;
    ...
    const ctx = document.getElementById("goldTransGraph").getContext("2d");
    new Chart(ctx, {
      type: 'scatter',
      data: {
        datasets: [
